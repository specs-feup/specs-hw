...
public class BTFWorker extends SpecsHwWorker {
    ...
    @Override
    public byte[] workInternal(String function, byte[] data,
        ...
        for (var detector : input.getDetectors()) {
            var bundle = BinaryTranslationFrontEndUtils.doBackend(
                    input.getProgram(), MicroBlazeElfStream.class, detector);
            bundle.toJSON();

            // save graph bundle
            output.addGraphs(bundle.getGraphs());

            // set instruction stream (this only needs to be done
            // once though, figure out a better way to do this later)
