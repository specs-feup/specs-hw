...
public class RiscvCDFGGenerationTest {
    ...
    public void exportInstructionCDFGAsDOT() throws IOException {
        ...
        InstructionCDFGDOTExporter.generateGraphvizFile(HardwareFolderGenerator.getBTFFolder(systemPath),
                this.instructionName, writer.toString());
        exp.emit((GeneralFlowGraph) this.icdfg, this.instructionName, writer, HardwareFolderGenerator
                .newFile(HardwareFolderGenerator.getBTFFolder(systemPath), this.instructionName, "dot"));
    ...
    public void generateHardwareModule() throws IOException {
        ...
        this.generatedModule
                .emit(HardwareFolderGenerator.newHardwareHDLFile(systemPath, this.generatedModule.getName(), "sv"));
    ...
    public void generateHardwareModuleValidationData() throws IOException {
        ...
        Map<Map<String, Number>, Map<String, Number>> validationData = PseudoInstructionCalculatorGenerator
                .generateValidationData(instructionPseudoCode.getParseTree(), icdfg.getDataInputsReferences(),
                        moduleTestbenchSamples);

        System.out.print("Generating HW module testbench validation input memory file...");
        HardwareFolderGenerator.newHardwareTestbenchFile(systemPath, "input", "mem")
                .write(PseudoInstructionCalculatorGenerator.generateHexMemFile(validationData.keySet()).getBytes());
        System.out.println("\tDONE");

        System.out.print("Generating HW module testbench validation output memory file...");
        HardwareFolderGenerator.newHardwareTestbenchFile(systemPath, "output", "mem")
                .write(PseudoInstructionCalculatorGenerator.generateHexMemFile(validationData.values()).getBytes());
        System.out.println("\tDONE");
    }

    public void generateHardwareModuleTestbench() throws IOException {
        System.out.print("Generating HW module testbench...");
        HardwareTestbenchGenerator.generate(generatedModule, moduleTestbenchSamples, "input.mem", "output.mem").emit(
                HardwareFolderGenerator.newHardwareTestbenchFile(systemPath, generatedModule.getName() + "_tb", "sv"));
        System.out.println("\tDONE");
    }

    public void generateVerilatorTestbench() throws IOException {
        System.out.print("Generating HW module Verilator testbench...");
        VerilatorTestbenchGenerator.emit(
                HardwareFolderGenerator.newHardwareTestbenchFile(systemPath, this.instructionName + "_tb", "cpp"),
                generatedModule.getName(), moduleTestbenchSamples);
        System.out.println("\tDONE");
    }
