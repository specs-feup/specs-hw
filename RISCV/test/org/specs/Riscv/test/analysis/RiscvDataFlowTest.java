...
public class RiscvDataFlowTest {
    ...
    public List<Instruction> getBasicBlock(RiscvELFProvider elf, long start, long end) {
        var dump = new RiscvELFDump(new RiscvObjDumpProvider(elf));
    ...
    public void testScheduling() {
        ...
        for (var elf : elfs.keySet()) {
            ...
            for (var bound : elfs.get(elf)) {
                ...
                var start = bound[0];
                var end = bound[1];
                var bb = getBasicBlock((RiscvELFProvider) elf, start, end);
