...
function installToolchain {
	...
	if [ "$1" == "microblaze" ]; then
		...
		CONFIGUREFLAGS="--target=aarch64-xilinx-elf --disable-silent-rules --disable-dependency-tracking --with-gnu-ld --enable-shared --enable-languages=c,c++ --enable-threads=posix --enable-multilib --enable-c99 --enable-long-long --enable-libstdcxx-pch --program-prefix=aarch64-xilinx-elf- --without-local-prefix --disable-install-libiberty --enable-lto --disable-libssp --enable-libitm --disable-bootstrap --with-system-zlib --enable-linker-build-id --with-ppl=no --with-cloog=no --enable-checking=release --enable-cheaders=c_global --without-isl --enable-poison-system-directories --enable-nls --with-glibc-version=2.28 --enable-initfini-array --without-headers --with-newlib --disable-libstdcxx-pch --disable-threads --enable-plugins --with-gnu-as --disable-libitm --disable-multiarch --with-arch=armv8-a --enable-multilib --with-sysroot=/not/exist"
	else
		echo "Invalid arch" >&3
		exit 1;
	fi
